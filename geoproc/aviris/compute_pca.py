...
pca_input = filtered_input_bands.fillna(0.0).values.reshape( filtered_input_bands.shape[0], n_samples  ).transpose()
...
scale = reduced_data.std( dim=["x","y"] )
reduced_data_scaled = reduced_data/scale
reduced_data_scaled.name = "reduced_data"
scale.name = "scale"
...
result_file = DATA_DIR + f"/pca-{n_components}.nc"
