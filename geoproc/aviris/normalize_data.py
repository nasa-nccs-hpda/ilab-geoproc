...
def normalize( input_bands: xa.DataArray, name: str, method: str ) -> Tuple[xa.DataArray,xa.DataArray]:
    scale = input_bands.std( dim=['x','y'] ) if method == "std" else np.abs(input_bands).mean( dim=['x','y'] )
...
if norm_band_data:
    ...
    band_data, scale =  normalize( input_bands, "band_data", norm_method )
...
if norm_training_data:
    ...
    band_data, scale = normalize( target_band, "band_data", norm_method )
